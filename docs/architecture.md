...
├── oxidian-core       ← error types, models, Snowflake
├── oxidian-gateway    ← WebSocket connection, heartbeat, event dispatch
├── oxidian-http       ← REST client, rate limiting
├── oxidian-interactions  ← slash commands (early)
└── oxidian-voice      ← voice (early)
```

`oxidian` is a thin re-export crate. It pulls everything together and provides the `Bot`, `BotBuilder`, `Context`, and `EventHandler` types that most bots will interact with directly.
...
Each event task gets a cloned `Context` (just an `Arc` clone, cheap), a cloned `Arc<dyn EventHandler>`, and a cloned `Arc<CommandRegistry>`. There's no global state.
